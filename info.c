...
int display()
{
	...
	int ID, price, quantity;
	for(int i = 0; i < len; i++)								//Displaying contents of array "products" one by one
	{
		char name[30]; char category[30];
		ID = (*(products+i)).id;
		price = (*(products+i)).price;
...
int filter_search()
{
	printf("Enter '1' for yes and any other number for no for all the following questions.\n");
        ...
        int fprice = 100000; int fquantity = -1; int j = 0;						//Set filter quantity to -1
        ...
        printf("Do you want to filter according to price(all items less than entered price will be displayed) : ");
        ...
        printf("Do you want to filter according to quantity(all items with quantity greater than entered quantity will be displayed) : ");
	...
	printf("Keep in mind, any other saved data will be truncated and saved in 'saved.txt' : \n");
